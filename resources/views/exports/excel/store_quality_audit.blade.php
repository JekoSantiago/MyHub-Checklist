...
<table>
    <thead>
        <tr>
            <th style="border: 1px solid #000000;" align="center" colspan="2">MONITORING / CHECKLISTS IMPLEMENTATION</th>
            ...
            @foreach ($mc['Items'] as $mci)
                <tr>
                    ...
                    @foreach ($mci['Options'] as $mcio)
                        @php $mcio->isSelected == 1 ? $color = '#80ffaa' : $color = '' @endphp
                        <td style="background-color: {{ $color }}; border: 1px solid #000000;">{{ $mcio->OptionName }}</td>
                    @endforeach
                </tr>
            @endforeach
        @endforeach
            ...
            @foreach ($fc['Items'] as $fci)
                <tr>
                    ...
                    @foreach ($fci['Options'] as $fcio)
                        @php $fcio->isSelected == 1 ? $color = '#80ffaa' : $color = '' @endphp
                        <td style="background-color: {{ $color }}; border: 1px solid #000000;">{{ $fcio->OptionName }}</td>
                    @endforeach
                </tr>
            @endforeach
        @endforeach
...
<table>
    <thead>
        <tr>
            <th colspan="4">OTHER COMMENTS / REMARKS</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td height="50" colspan="4">{{ $checklist[0]->Remarks }}</td>
        </tr>
    </tbody>
</table>
...
<table>
    ...
    <tbody>
        <tr>
            <td>{{ $checklist[0]->AuditByName }}</td>
            <td>{{ $checklist[0]->AC }} / {{ $checklist[0]->AMAppName }}</td>
            <td>{{ $checklist[0]->AcceptedByName }}</td>
        </tr>
    </tbody>
</table>
